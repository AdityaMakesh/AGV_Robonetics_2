...
float left_speed_rad_s = ((2*cmd_vel->linear.x-RABBIT_AXLE_LENGTH*cmd_vel->angular.z)/(RABBIT_AXLE_LENGTH*RABBIT_DIAMETER)); // Left wheel ve
float right_speed_rad_s = ((2*cmd_vel->linear.x+RABBIT_AXLE_LENGTH*cmd_vel->angular.z)/(RABBIT_AXLE_LENGTH*RABBIT_DIAMETER));        // Right

//The value range is form 0 - 200 rps coreesponding scale value is 0 to 255 in controller
//left_speed_rpm = abs((left_speed_rad_s*60)/(2*3.14));
//right_speed_rpm =abs((right_speed_rad_s*60)/(2*3.14));

left_speed_rpm = ((left_speed_rad_s*60)/(2*3.14))*2.55;
right_speed_rpm =((right_speed_rad_s*60)/(2*3.14))*2.55;

if(abs(left_speed_rpm) > RABBIT_MAX_RPM)	left_speed_rpm =(int) (left_speed_rpm/abs(left_speed_rpm))*RABBIT_MAX_RPM;
if(abs(right_speed_rpm) > RABBIT_MAX_RPM)	right_speed_rpm =(int) (right_speed_rpm/abs(right_speed_rpm))*RABBIT_MAX_RPM;

if(rabbit_left->writeMotorSpeed(left_speed_rpm*-1) == 0) {ROS_INFO("Left velocity:%i rps L:%f A:%f :%f",left_speed_rpm, cmd_vel->linear.x, cmd_vel->angular.z, left_speed_rad_s);}
if(rabbit_right->writeMotorSpeed(right_speed_rpm) == 0) {ROS_INFO("Right velocity:%i rps L:%f A:%f :%f",right_speed_rpm, cmd_vel->linear.x, cmd_vel->angular.z, right_speed_rad_s);}
...
int main(int argc, char** argv)
{
	...
	n.getParam("pnode/rabbit/port_a", port_a);
	n.getParam("pnode/rabbit/port_b", port_b);
	n.getParam("pnode/rabbit/cmd_vel_time_out_disable", cmd_vel_time_out_disable);
	n.getParam("pnode/rabbit/cmd_vel_time_out", cmd_vel_time_out);
...
//end of modification
	...
	if(( rabbit_left->openSerialPort() == 0)&&( rabbit_right->openSerialPort() == 0))
		ROS_INFO("Connected to eyantra base robot with MA:%s and MB:%s with timeout of %f and %s",port_a.c_str(),port_b.c_str(), cmd_vel_time_out, cmd_vel_time_out_disable ? "False":"True");
	else
	{
	...
	while(n.ok())
	{
		...
		ROS_INFO("EA:%lli and EB:%lli last_X:%lli and last_Y:%lli",left_encoder, right_encoder, last_left_encoder, last_right_encoder);
		...
		long long int ldiff = abs(abs(left_encoder) - abs(last_left_encoder));
		long long int rdiff = abs(abs(right_encoder) - abs(last_right_encoder));
		...
		if((ros::WallTime::now().toSec() - last_cmd_vel_received) > cmd_vel_time_out)
		{
		rabbit_left->writeMotorSpeed(0);
		rabbit_right->writeMotorSpeed(0);
		last_cmd_vel_received = ros::WallTime::now().toSec() + 5;
