...
float kpw,kpx,kpy,kdw,kdx,kdy,yawd,xd,yd,wd;
...
n.getParam("control/wd", yawd);
n.getParam("control/xd", xd);
n.getParam("control/yd", yd);

ros::Rate loop_rate(5);

ROS_INFO("Control constants: [kpx:%f kpy: %f kpw: %f  kdx:%f kdy: %f kdw: %f xd:%f  yd:%f wd:%f ]", kpx, kpy, kpw, kdx, kdy, kdw, xd, yd, yawd);
...
while(ros::ok())
{
   ...
   */
  u = sqrt(vx*vx+vy*vy);
  wd = atan2((yd-py),(xd-px));

//  Fx = (kpx * cos(yaw) * (xd-px) + kpy * sin(yaw) * (yd-py) - kdx * u);
//  Mz = (kpw * (yawd - yaw) - kdw * (wd-wz));
  ...
  base_cmd.angular.z = (f2-f1)*(RABBIT_DIAMETER/2);
...
ROS_INFO("ROBOT Pos: [L:%f: %f A:%f wd: %f f1:%f f2:%f :: %f : %f]", px, py, yaw, wd, f1, f2, base_cmd.linear.x, base_cmd.angular.z);
