...
int main(int argc, char **argv)
{
   ...
   */
  ros::init(argc, argv, "array_param_test");
...
std::vector<double> my_int_list;
n.getParam("my_int_list", my_int_list);

ROS_INFO("param list size::%i",(int)my_int_list.size());

for(unsigned int i=0; i < my_int_list.size();i++)
{
	ROS_INFO("Param [%i]=%f",i,my_int_list[i]);
}
