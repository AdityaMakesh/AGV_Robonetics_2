...
{
	uint16_t tab_reg[32];
	ROS_INFO("Reached %i : %i : %i",msg->slaveid, msg->write_multiple_registers_start_address, msg->write_multiple_registers_count);
	...
	for(k=0; k < msg->write_multiple_registers.size(); ++k)
	{
	tab_reg[k] = msg->write_multiple_registers[k];
	ROS_INFO("Reached %i : %i",k,tab_reg[k]);
	}
...
int main(int argc, char **argv)
{
	...
	ros::ServiceServer writemultipleregisters_srv = n.advertiseService("mb_writemultipleregisters", writemultipleregisters);
	ros::Publisher readholdingregisters_pub = n.advertise<modbustest::readholdingregisters>("modbus_read_holding_registers", 1);
	ros::Publisher readinputregisters_pub = n.advertise<modbustest::readinputregisters>("modbus_read_input_registers", 1);
	ros::Subscriber sub = n.subscribe("modbus_writemultipleregisters", 1, modbus_writemultipleregisters);
	ros::Rate loop_rate(300);


	ROS_INFO("MODBUS communication intializing...");
	n.param<std::string>("modbus_server_node/modbus/port", port, "/dev/ttyUSB0");
	n.param<int>("modbus_server_node/modbus/baud", baud, 19200);
	n.param<int>("modbus_server_node/modbus/timeout", mbtoutsecs, 10);
	...
	n.getParam("modbus_server_node/modbus/read_holding_registers_slaveids", read_holding_registers_slaveids);

	std::vector<int> read_holding_registers_count;
	n.getParam("modbus_server_node/modbus/read_holding_registers_count", read_holding_registers_count);

	std::vector<int> read_holding_registers_start_address;
	n.getParam("modbus_server_node/modbus/read_holding_registers_start_address", read_holding_registers_start_address);

	std::vector<int> read_input_registers_slaveids;
	n.getParam("modbus_server_node/modbus/read_input_registers_slaveids", read_input_registers_slaveids);

	std::vector<int> read_input_registers_count;
	n.getParam("modbus_server_node/modbus/read_input_registers_count", read_input_registers_count);

	std::vector<int> read_input_registers_start_address;
	n.getParam("modbus_server_node/modbus/read_input_registers_start_address", read_input_registers_start_address);
	...
	if(( (read_input_registers_slaveids.size()) != (read_input_registers_count.size()) && (read_input_registers_slaveids.size()) != (read_input_registers_start_address.size()) ))
{
	...
	ROS_INFO("Trying to intialize communication with %i slaves with %i input registers and %i holding registers", read_holding_registers_slaveids.size(), read_holding_registers_count.size(), read_input_registers_count.size());
...
//	ROS_INFO("%s", msg.data.c_str());
	...
	//Resetting after raching maximum value
